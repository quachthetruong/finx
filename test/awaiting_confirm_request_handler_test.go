...
func TestAwaitingConfirmRequestHandler(t *testing.T) {
	t.Parallel()
	...
	t.Run(
		"Get all awaiting confirm requests success", func(t *testing.T) {
			...
			ginCtx.Request = gintest.MustMakeRequest("GET", "/api/v1/awaiting-confirm-requests?sort=id", nil)
			...
			assert.Equal(t, "5, 78", testhelper.GetString(body, "data", "[0]", "loanPackageIds"))
			assert.Equal(t, "CA2", testhelper.GetString(body, "data", "[0]", "latestUpdate", "category"))
			assert.Equal(t, "", testhelper.GetString(body, "data", "[1]", "loanPackageIds"))

			ginCtx, _, recorder = gintest.GetTestContext()
			ginCtx.Request = gintest.MustMakeRequest(
				"GET", "/api/v1/awaiting-confirm-requests?sort=id&latestUpdateCategories=CA2", nil,
			)
			handler.GetAll(ginCtx)
			result = recorder.Result()
			body = gintest.ExtractBody(result.Body)
